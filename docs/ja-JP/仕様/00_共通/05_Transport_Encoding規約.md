...
ただし transport = ui_copy が適用される場合、
以下の規則が優先される。

- 各成果物は独立したコードブロックとして連続提示されなければならない。
- コードブロック以外の文字列を成果物間または前後に出力してはならない。

この制約は ui_copy の UI 安定性要件に基づくものであり、
他の transport 方式（true_out / download）には適用されない。
