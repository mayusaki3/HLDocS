...
### 技術検証時のWorkLog管理 (SHOULD)

技術検証を継続して実施する場合、
LLMは WorkLog に以下を記録することを推奨する（SHOULD）。
...
次成果物:
linux_webview.rs 差し替え版

技術検証において、
WorkLogは技術検証中の作業状態および作業記録の正本として扱うことを推奨する（SHOULD）。

技術検証において、
現在工程および次工程の管理は
WorkLog を正本とすることを推奨する（SHOULD）。

LLMは技術検証開始時および継続時に、
WorkLog の内容を参照してよい（MAY）。
...
技術検証中は、
検証結果文書へ都度反映することを必須としない。

利用者から検証結果が返却され、
LLMが評価を完了した場合、
LLMはWorkLogへ検証内容および評価結果を追記することを推奨する（SHOULD）。

技術検証の完了時、
または利用者が要求した場合、
LLMはWorkLogに蓄積された記録を検証結果文書へ反映することを推奨する（SHOULD）。

技術検証が完了した場合、
WorkLogのみを更新して終了することは推奨しない（SHOULD NOT）。

LLMは検証結果文書への反映、
または反映候補の提示を行うことを推奨する（SHOULD）。
