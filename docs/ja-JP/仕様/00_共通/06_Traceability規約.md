...
- HLDocS成果物生成時は、HLDocS仕様を最優先とし、
  UI仕様・ツール仕様（file_search 等）よりも優先される（MUST）
...
### 6.1 成果物本文における参照形式制約（MUST）

本規約における「参照」とは、
機械的に識別可能な識別子形式（doc_id#sec_id 等）による
リンク表現を指す。

自然文による説明は参照に含めない。

HLDocS成果物本文における参照は、以下の形式のみを許可する。

- doc_id#sec_id

上記以外のすべての参照形式の生成を禁止する（MUST NOT）。

禁止対象には以下を含む（ただしこれに限らない）：
...
参照が必要であるが sec_id が存在しない場合、
LLMは参照形式（doc_id#sec_id）を生成してはならない（MUST）。

この場合、当該内容は参照を伴わない自然文として記述してよい（MAY）。

本制約は、説明文・出典表記・補足であっても例外なく適用される（MUST）。

本制約は、HLDocS成果物本文およびテンプレート範囲内に適用される。

人間向けナビゲーションリンク（Markdownリンク）は、
機械参照とは別目的であるため、本制約の対象外とする。
